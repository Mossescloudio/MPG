...
cts.GenerateSubtab = (form, myParams) => {
    form.addSubtab({ id: "subtab_table", label: "sublist Table", });
    ...
    for (let i = 0; i < EMIValues.length; i++) {
        totalInterest += EMIValues[i].interest;
        totalEMI += EMIValues[i].emi;
        totalRepayed += EMIValues[i].capitalRepay;

        sublist.setSublistValue({
            id: 'custpage_sn',
            line: i,
            value: EMIValues[i].no.toString()
        });

        sublist.setSublistValue({
            id: 'custpage_principal',
            line: i,
            value: EMIValues[i].principal.toFixed(2)
        });
        sublist.setSublistValue({
            id: 'custpage_interest',
            line: i,
            value: EMIValues[i].interest.toFixed(2)
        });
        sublist.setSublistValue({
            id: 'custpage_installment',
            line: i,
            value: EMIValues[i].emi.toFixed(2)
        });
        sublist.setSublistValue({
            id: 'custpage_capital_repay',
            line: i,
            value: EMIValues[i].capitalRepay.toFixed(2)
        });
        sublist.setSublistValue({
            id: 'custpage_balance',
            line: i,
            value: EMIValues[i].balance
        });
    ...
    sublist.setSublistValue({
        id: 'custpage_installment',
        line: EMIValues.length,
        value: totalEMI.toFixed(2)
    });
    sublist.setSublistValue({
        id: 'custpage_capital_repay',
        line: EMIValues.length,
        value: totalRepayed.toFixed(2)
    });
    sublist.setSublistValue({
        id: 'custpage_balance',
        line: EMIValues.length,
